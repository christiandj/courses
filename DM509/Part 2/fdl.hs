...
apply (x:xs) rules = checkRule x rules ++ apply xs rules
apply _ _ = []

-- helper function to check if letters have applicable rules
checkRule :: Char -> [Rule] -> State
checkRule x (Rule ls rs:ys) = if x == ls then rs else checkRule x ys
checkRule x [] = [x]

-- expand to target depth
expand :: State -> [Rule] -> Int -> State
expand state rules depth = if depth > 0 then expand (apply state rules) rules (depth-1) else state

-- convert fractal into sequence of turtle graphics commands
process :: Fractal -> [Command]
process (start, rules, m, depth, len) = map m (expand start rules depth)
...
interpret (("rule":xs):yss)    start rules m depth len = interpret yss start (rules ++ [interpretRule xs]) m depth len where
  interpretRule (l:"->":rs) = Rule (head l) (concat rs)
interpret (("start":xs):yss)   _     rules m depth len = interpret yss (concat xs) rules m depth len
